...
This outputs `_mdpro-review.js`, `_mdpro-review.css`, and `web/editor.*` to `markdown_pro/`.
...
- Generates `markdown_pro/shiki-data.json` (version, languages, themes)
- Updates `markdown_pro/config.json` with defaults
- Cleans stray `_mdpro-lang-*.js` / `_mdpro-theme-*.js` files
