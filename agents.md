...
This project is an Anki add-on that creates two note types (`Markdown Pro` and `Markdown Pro Cloze`), stores note content as plain markdown, renders fields as markdown during review, and adds Shiki-based syntax highlighting for code.
...
bun run package     # Build and create markdown-pro.ankiaddon
...
markdown_pro/           Python add-on package plus built assets
markdown_pro/templates/ Note type templates used by Front/Back and Cloze cards
...
- Edit: `src/*`, `markdown_pro/*.py`, `markdown_pro/templates/*`, `scripts/*`, docs, tests, skill files
- Generated: `markdown_pro/_mdpro-review.js`, `markdown_pro/_mdpro-review.css`, `markdown_pro/web/*`, `markdown_pro/shiki-data.json`, `markdown_pro/config.json`
...
`markdown_pro/__init__.py` runs on profile load and:
...
3. creates or updates the `Markdown Pro` and `Markdown Pro Cloze` note types
4. registers `web/editor.js` and `web/editor.css` for the editor webview
5. registers the settings dialog

### Editor Save Path

`editor_will_munge_html` converts simple HTML back into markdown before save for both `Markdown Pro` and `Markdown Pro Cloze` notes. That keeps stored field content clean even if Anki emits HTML while editing.

### Card Rendering

`src/render.ts` powers reviewed cards:

- reads config from the injected `#mdpro-config` JSON script
...
`src/editor.ts` activates for both `Markdown Pro` and `Markdown Pro Cloze` note types. It uses:
...
- `markdown_pro/shiki-data.json`
- `markdown_pro/config.json`

`markdown_pro/shiki.py` owns language and theme download logic. Its important layers are:
...
- preserve the skill requirement to use `Markdown Pro` or `Markdown Pro Cloze` note types
...
- The note type name must stay `Markdown Pro`. The cloze variant must stay `Markdown Pro Cloze`.
...
- If you change template structure, update the relevant templates in `markdown_pro/templates/`.
