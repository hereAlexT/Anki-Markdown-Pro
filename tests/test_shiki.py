...
class TestRewriteLangImports:
    def test_rewrites(self, shiki):
        ...
        assert result == 'import t from"./_mdpro-lang-javascript.js";import e from"./_mdpro-lang-css.js";'
...
class TestNeedsRedownload:
    ...
    def test_broken(self, shiki, tmp_path):
        (tmp_path / "_mdpro-lang-html.js").write_text('import t from"./javascript.mjs";')
        s = shiki.ShikiStore(tmp_path)
        assert s.needs_redownload("html") is True

    def test_fixed(self, shiki, tmp_path):
        (tmp_path / "_mdpro-lang-html.js").write_text('import t from"./_mdpro-lang-javascript.js";')
        (tmp_path / "_mdpro-lang-javascript.js").write_text("var x;")
        s = shiki.ShikiStore(tmp_path)
    ...
    def test_missing_transitive_dep(self, shiki, tmp_path):
        ...
        (tmp_path / "_mdpro-lang-nginx.js").write_text('import t from"./_mdpro-lang-lua.js";')
        (tmp_path / "_mdpro-lang-lua.js").write_text('import t from"./_mdpro-lang-c.js";')
        s = shiki.ShikiStore(tmp_path)
...
class TestDownloadLang:
    def test_html(self, shiki, tmp_path):
        ...
        html = (tmp_path / "_mdpro-lang-html.js").read_text()
        assert 'from"./javascript.mjs"' not in html
        assert 'from"./_mdpro-lang-javascript.js"' in html
        assert (tmp_path / "_mdpro-lang-javascript.js").exists()
        assert (tmp_path / "_mdpro-lang-css.js").exists()

    def test_bash_alias(self, shiki, tmp_path):
        """Resolves alias, full grammar not stub."""
        s = shiki.ShikiStore(tmp_path)
        s.download_lang("bash")
        content = (tmp_path / "_mdpro-lang-bash.js").read_text()
...
class TestSync:
    def test_sync(self, shiki, tmp_path):
        ...
        assert not errors
        assert (tmp_path / "_mdpro-lang-python.js").exists()
        assert (tmp_path / "_mdpro-theme-vitesse-light.js").exists()
        assert (tmp_path / "_mdpro-theme-vitesse-dark.js").exists()
        ...
        (tmp_path / "_mdpro-lang-python.js").write_text('from"./broken.mjs";')
        downloaded3, _ = s.sync(config)
        assert "_mdpro-lang-python.js" in downloaded3
    ...
    def test_dep_failure_recovery(self, shiki, monkeypatch, tmp_path):
        ...
        assert errors
        assert (tmp_path / "_mdpro-lang-html.js").exists()
        assert not (tmp_path / "_mdpro-lang-javascript.js").exists()

        # Restore fetch, retry → recovers
        monkeypatch.setattr(shiki, "fetch_module", orig)
        downloaded, errors = s.sync(config)
        assert not errors
        assert "_mdpro-lang-html.js" in downloaded
        assert (tmp_path / "_mdpro-lang-javascript.js").exists()
    ...
    def test_transitive_dep_failure_recovery(self, shiki, monkeypatch, tmp_path):
        ...
        _, errors = s.sync(config)
        assert errors
        assert (tmp_path / "_mdpro-lang-nginx.js").exists()
        assert (tmp_path / "_mdpro-lang-lua.js").exists()
        assert not (tmp_path / "_mdpro-lang-c.js").exists()

        # Restore fetch, retry -> recovers transitive dep too
        monkeypatch.setattr(shiki, "fetch_module", orig)
        downloaded, errors = s.sync(config)
        assert not errors
        assert "_mdpro-lang-nginx.js" in downloaded
        assert (tmp_path / "_mdpro-lang-c.js").exists()
...
class TestCleanup:
    def test_dep_protected(self, shiki, tmp_path):
        ...
        assert "_mdpro-lang-javascript.js" not in removed
        assert (tmp_path / "_mdpro-lang-javascript.js").exists()
    ...
    def test_transitive_dep_protected(self, shiki, tmp_path):
        ...
        s = shiki.ShikiStore(tmp_path)
        (tmp_path / "_mdpro-lang-html.js").write_text('import t from"./_mdpro-lang-javascript.js";')
        (tmp_path / "_mdpro-lang-javascript.js").write_text('import t from"./_mdpro-lang-css.js";')
        (tmp_path / "_mdpro-lang-css.js").write_text("var x;")
        config = {"languages": ["html"], "themes": {"light": "a", "dark": "b"}}
    ...
    def test_unused_removed(self, shiki, tmp_path):
        ...
        s = shiki.ShikiStore(tmp_path)
        (tmp_path / "_mdpro-lang-ruby.js").write_text("var x;")
        config = {"languages": [], "themes": {"light": "a", "dark": "b"}}
        removed = s.cleanup(config)
        assert "_mdpro-lang-ruby.js" in removed
    ...
    def test_orphan(self, shiki, tmp_path):
        ...
        removed = s.cleanup(config)
        assert "_mdpro-lang-html.js" in removed
        assert "_mdpro-lang-javascript.js" in removed
    ...
    def test_transitive_orphan(self, shiki, tmp_path):
        ...
        assert "_mdpro-lang-html.js" in removed
        assert "_mdpro-lang-javascript.js" in removed
        assert "_mdpro-lang-css.js" in removed

    def test_cycle(self, shiki, tmp_path):
        """Cycles do not loop and configured members stay reachable."""
        s = shiki.ShikiStore(tmp_path)
        (tmp_path / "_mdpro-lang-a.js").write_text('import t from"./_mdpro-lang-b.js";')
        (tmp_path / "_mdpro-lang-b.js").write_text('import t from"./_mdpro-lang-a.js";')
    ...
    def test_theme_removed(self, shiki, tmp_path):
        ...
        s = shiki.ShikiStore(tmp_path)
        (tmp_path / "_mdpro-theme-monokai.js").write_text("var x;")
        config = {"languages": [], "themes": {"light": "vitesse-light", "dark": "vitesse-dark"}}
        removed = s.cleanup(config)
        assert "_mdpro-theme-monokai.js" in removed


class TestDebug:
    def test_local_deps(self, shiki, tmp_path):
        s = shiki.ShikiStore(tmp_path)
        (tmp_path / "_mdpro-lang-html.js").write_text(
            'import t from"./_mdpro-lang-javascript.js";import e from"./_mdpro-lang-css.js";'
        )
    ...
    def test_debug_data(self, shiki, tmp_path):
        ...
        (tmp_path / "_mdpro-lang-html.js").write_text('import t from"./_mdpro-lang-javascript.js";')
        (tmp_path / "_mdpro-lang-javascript.js").write_text("var x;")
        (tmp_path / "_mdpro-theme-vitesse-dark.js").write_text("var x;")
        config = {
...
class TestOnlineDownload:
    ...
    def test_bash_alias(self, shiki_online, tmp_path):
        s = shiki_online.ShikiStore(tmp_path)
        s.download_lang("bash")
        assert len((tmp_path / "_mdpro-lang-bash.js").read_text()) > 200
