...
- **MUST** use the `Markdown Pro` note type for basic cards or `Markdown Pro Cloze` for cloze deletions
...
Use **Markdown Pro** (Front/Back) for question-answer pairs where you write a specific question.
Use **Markdown Pro Cloze** (Text/Extra) for fill-in-the-blank cards where hiding parts of a statement is more natural than writing a question.

## Cloze Cards

Use `Markdown Pro Cloze` for fill-in-the-blank cards. Fields are `Text` and `Extra`.
...
```bash
# Basic cloze
$ANKI add "MyDeck" "Markdown Pro Cloze" '{"Text":"The {{c1::CPU}} executes {{c2::instructions}}.","Extra":"Basic computer architecture."}' --tags "cs"

# Cloze with hint
$ANKI add "MyDeck" "Markdown Pro Cloze" '{"Text":"{{c1::JavaScript::language}} was created in {{c2::1995::year}}.","Extra":"Created by Brendan Eich."}' --tags "history"

# Cloze with blur
$ANKI add "MyDeck" "Markdown Pro Cloze" '{"Text":"The speed of light is {{c1::299,792,458 m/s::blur}}.","Extra":"Often approximated as 3 x 10^8 m/s."}' --tags "physics"
```
...
| `fields`   | Fields for a model   | `$ANKI fields "Markdown Pro"`                                                  |
| `find`     | Search notes         | `$ANKI find "deck:Spanish tag:verb"`                                            |
| `info`     | Note details         | `$ANKI info 1234 5678`                                                          |
| `add`      | Add one note         | `$ANKI add "My Deck" "Markdown Pro" '{"Front":"Q","Back":"A"}' --tags "t1 t2"` |
| `add-bulk` | Add many notes       | `$ANKI add-bulk "My Deck" "Markdown Pro" '[{"Front":"Q1","Back":"A1"}]'`       |
...
$ANKI fields "Markdown Pro"        # verify field names for selected model
$ANKI find "deck:MyDeck front:test" # check for duplicates
$ANKI add "MyDeck" "Markdown Pro" '{"Front":"...","Back":"..."}'
